...
CREATE TABLE purchase_details(purchase_detail_code VARCHAR2(15) NOT NULL
                             ...
                             ,item_tax NUMBER(3
                                    ,2));
ALTER TABLE purchase_details add(incoming_flag CHAR(1));
...
drop TABLE purchase_detail_temp purge;
CREATE TABLE purchase_detail_temp(tmp_detail_code VARCHAR2(15)
                                 ...
                                 ,tmp_item_tax NUMBER(3
                                        ,2));

---------------------------------------
-------<기초정보: 재고정보>
---------------------------------------
drop TABLE stocks_in_out cascade constraints;
CREATE TABLE stocks_in_out(stock_id NUMBER NOT NULL
                          ...
                          ,out_qty NUMBER
                          ,txn_date DATE);
CREATE UNIQUE INDEX stocks_id_uk ON stocks_in_out(stock_id);
COMMENT ON column stocks_in_out.item_code IS '품목코드';
COMMENT ON column stocks_in_out.deal_date IS '거래날짜';
COMMENT ON column stocks_in_out.dealer_code IS '구매, 판매번호';
COMMENT ON column stocks_in_out.in_qty IS '입고수량';
COMMENT ON column stocks_in_out.out_qty IS '출고수량';
...
-----------------------sequences-------------
CREATE sequence sales_detail_seq START
    WITH 1000000000 increment BY 1;

CREATE sequence purchase_detail_seq START
    WITH 1000000001 increment BY 1;

CREATE sequence stocks_in_out_seq START
    WITH 1000000001 increment BY 1;
